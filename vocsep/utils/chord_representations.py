...
def time_divided_tsv_to_note_array(time_divided_tsv_path, transpose=False, version="v1.0.0"):
    ...
    if transpose:
        ...
        data = [createfunction(filtered_df, time_signature)]
        for interval in transpositions:
            data.append(createfunction(filtered_df, time_signature, interval=interval))
        return data
    else:
...
def time_divided_tsv_to_part(time_divided_tsv_path, transpose=False, version="v1.0.0"):
    ...
    if transpose:
        ...
        for (note_array, labels) in X:
            note_array, labels = tie_consecutive_notes(note_array, labels)
            note_array = create_divs_from_beats(note_array)
            new_X.append((note_array, labels))
        return new_X
    else:
        note_array, labels = time_divided_tsv_to_note_array(time_divided_tsv_path, transpose=transpose, version=version)
        note_array, labels = tie_consecutive_notes(note_array, labels)
        note_array = create_divs_from_beats(note_array)
        return note_array, labels
